...
export function phenology(chart, data, textId) {
  ...
  //console.log("months", fs, fe, ls, le)
  ...
  // Source
  if (textId) {
    const source = `Data for flower phenology from ${data[0].phenFlowerRef}.</br>Data for leafing phenology from ${data[0].phenLeafRef}.`
    $(`#${textId}`).html(source)
  }

  const metrics = []
  if (ls && le) {
    metrics.push({ prop: 'band2', label: 'In leaf', colour: '#009900', opacity: 0.5, svg: svgLeaf })
  }
  if (fs && fe) {
    metrics.push({ prop: 'band1', label: 'Flowering', colour: '#ff9900', opacity: 0.5, svg: svgFlower })
  }
