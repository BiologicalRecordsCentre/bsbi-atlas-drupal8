(function (global, factory) {
  ...
  function getCSV(identifier) {
    var file = "".concat(bsbiDataAccess.bsbiDataRoot).concat(identifier.replace(".", "_"), ".csv");
    return file;
  }
    ...
    return new Promise(function (resolve, reject) {
      ...
      d3__namespace.csv(getCSV(identifier), function (r) {
        if (r.hectad) {
          ...
          var atlasstatus = r.atlasstatus ? r.atlasstatus : 'missing'; // Count the occurrences in each date category
  ...
  function distAllClassesTetrad(identifier) {
    ...
    return new Promise(function (resolve, reject) {
      d3__namespace.csv(getCSV('tetrad/' + identifier), function (r) {
        if (r.gr) {
          return {
            gr: r.gr,
  ...
  function distAllClassesMonad(identifier) {
    return new Promise(function (resolve, reject) {
      d3__namespace.csv(getCSV('monad/' + identifier), function (r) {
        if (r.gr) {
          return {
            gr: r.gr,
            shape: 'square',
  ...
  function setBaseMetaTags(_$) {
    $$1 = _$;
    addMetaTags$1('title', 'BSBI Online Atlas 2020');
    addMetaTags$1('authors', 'Stroh, P. A., Humphrey, T., Burkmar, R. J., Pescott, O. L., , Roy, D.B., and Walker, K. J.');
    addMetaTags$1('author', 'Stroh, P. A.');
    addMetaTags$1('author', 'Humphrey, T.');
    addMetaTags$1('author', 'Burkmar, R. J.');
    addMetaTags$1('author', 'Pescott, O. L.');
    addMetaTags$1('author', 'Roy, D. B.');
    addMetaTags$1('author', 'Walker, K. J.');
    addMetaTags$1('year', '2022');
    addMetaTags$1('url', location.origin + '/atlas');
  }

  function addMetaTags$1(type, value, update) {
    var addHeadTag = function addHeadTag(name, content, update) {
      if (update) {
        $$1('meta[name="' + name + '"').attr('content', content);
      } else {
        $$1('head').append('<meta name="' + name + '" content="' + content + '" />');
      }
  ...
  function createPhenology(_$, sel) {
    ...
    var $phenFlexParent = $('<div>').appendTo($(sel));
    $phenFlexParent.attr('class', 'phenRow');
    var $phenFlexLeft = $('<div>').appendTo($phenFlexParent);
    $phenFlexLeft.attr('class', 'phenColumn');
    var $phenFlexRight = $('<div>').appendTo($phenFlexParent);
    $phenFlexRight.attr('class', 'phenColumn');
    var $apparency = $('<div>').appendTo($phenFlexLeft);
    ...
    var $phenology = $('<div>').appendTo($phenFlexLeft);
    ...
    var $phenSource = $('<div>').appendTo($phenFlexLeft);
    ...
    var $apparencyByLat = $('<div>').appendTo($phenFlexRight);
    ...
    $('.brc-chart-phen1').css('overflow', 'visible'); // Chart line width - not currently a chart option
  ...
  function main($, drupalSettings) {
    ...
    $(document).ready(function () {
      ...
      mainAtlasContent(tabs); // Taxon selection control

      taxonSelectors('.bsbi-atlas-taxon-selector'); // Navigation block

      navigationBlock('.bsbi-atlas-navigation'); // Devel block
    ...
    function mainAtlasContent(tabs) {
      ...
      $('#main-atlas-content').html(null); // Generate main content with or without tabs
    ...
    function sectionEnd($sect, tabs) {
      if (!tabs) {
        ...
        $aLink = $('<a>').appendTo($pLink); //$aLink.html('&lArr;')
    ...
    function createMaps(selector) {
      ...
      console.log(mapTypesSel); // Basemaps
      ...
      staticMap.basemapImage('colour_elevation', true, rasterRoot + 'colour_elevation.png', rasterRoot + 'colour_elevation.pgw'); //staticMap.basemapImage('greyelev', false)
      // Callbacks for slippy maps

      function startLoad() {
        document.getElementById('atlas-loader').style.display = 'inline-block'; //document.getElementById('dev-download-time').innerHTML= "Downloading data..."
      }

      function endLoad() {
        document.getElementById('atlas-loader').style.display = 'none'; //var t2 = Math.floor(Date.now() / 100)
      ...
      function startDraw() {
        ...
        document.getElementById('atlas-loader').style.display = 'inline-block'; //document.getElementById('dev-display-time').innerHTML= "Displaying data..."
      }
    ...
    function taxonSelectors(selector) {
      ...
      d3.csv(taxaNoStatusCsv).then(function (data) {
        taxaNoStatusList = data.map(function (d) {
          return d['ddb id'];
        });
      }); // Overall control container
      ...
      $sel.addClass('atlas-taxon-selector-sel'); // Slider
      ...
      d3.csv(taxaCsv).then(function (data) {
        ...
        $sel.attr('title', 'Select a taxon to display'); //$sel.attr('data-width', '100%')
        ...
        $sel.on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
          ...
          currentTaxon.tetrad = $(this).find(":selected").attr("data-tetrad"); //currentTaxon.monad = $(this).find(":selected").attr("data-monad")

          var hybrid = taxaHybridList.find(function (h) {
            return h.taxon === currentTaxon.identifier;
          });
          currentTaxon.parent1 = hybrid ? hybrid.parent1 : '';
          currentTaxon.parent2 = hybrid ? hybrid.parent2 : ''; //$('.bsbi-selected-taxon-name').html(currentTaxon.name)
    ...
    function changeEcology() {
      if (!currentTaxon.identifier) return; // Apparency all
    ...
    function changeCaption() {
      ...
      $caption.html('');
      d3.csv(captionRoot + currentTaxon.identifier.replace(/\./g, "_") + '.csv?prevent-cache=09092021').then(function (d) {
        // Set taxon name
